		...
		<form id="form" method="post" action="<?php echo base_url('core/users_save') ?>">
			<div class="row">
				<div class="col-sm-6 col-md-6 col-lg-6">
					...
					<input value="<?php echo $users->user_id?>" required="" type="hidden" autocomplete="off" class="form-control" name="user_id">
				...
				<div class="col-sm-6 col-md-6 col-lg-6">
					...
					<select required="" autocomplete="off" class="select2 form-control" name="user_group_id">
						<?php foreach ($_sys_group as $key => $var): ?>
							<option <?php echo ($var['group_id'] == $users->user_group_id) ? 'selected' : '' ?> value="<?php echo $var['group_id'] ?>"><?php echo $var['group_label'] ?></option>
						<?php endforeach ?>
					</select>
				</div>
				<div class="col-sm-6 col-md-6 col-lg-6">
					...
					<select required="" autocomplete="off" class="form-control" name="user_status_id">
						<option <option <?php echo ($var == $users->user_status_id) ? 'selected' : '' ?> value="1">Aktif</option>
						<option <option <?php echo ($var == $users->user_status_id) ? 'selected' : '' ?> value="0">Non-Aktif</option>
					</select>
				</div>
			</div>
			<hr>
