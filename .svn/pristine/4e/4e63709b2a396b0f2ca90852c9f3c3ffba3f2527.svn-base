...
<script type="text/javascript">
	$(document).ready(function() {
		$('#tabel-data').dataTable({
			ajax:{url:"<?php echo base_url('core/navbars_fetch') ?>",dataSrc:"data","type": "POST"},
			processing: true,
        	serverSide: true,
			columns: [
		...
		$('#form-navbar-manage').submit(function(event) {
			$.post('<?php echo base_url('core/navbars_save') ?>', $(this).serialize(), function(response, textStatus, xhr) {
		...
		$('#form-navbar-delete').submit(function(event) {
			$.post('<?php echo base_url('core/navbars_delete') ?>', $(this).serialize(), function(response, textStatus, xhr) {
				if (response.status == true) {
					toastr.success(response.msg);
					setTimeout(function() {
						window.location.reload();
	...
	function dt_add(t) {
		$.get('<?php echo base_url('core/navbars_modal/modal_add') ?>').done(function(data) {
	...
	function dt_edit(t) {
		$.get('<?php echo base_url('core/navbars_modal/modal_edit') ?>', {'navbar_id':$(t).attr('target-id')}).done(function(data) {
			$('#modal-body-navbar').html(data);
			$('#modal_navbar').modal('show');
		});
	}
	...
	function dt_delete(t){
		$.get('<?php echo base_url('core/navbars_modal/modal_delete') ?>', {'navbar_id':$(t).attr('target-id')}).done(function(data) {
