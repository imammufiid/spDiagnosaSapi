...
<script type="text/javascript">
	$(document).ready(function() {
		var tabel = $('#tabel-data').DataTable({
			ajax: {
				url: "<?php echo base_url('atlet/fetch') ?>",
				dataSrc: "data",
				"type": "POST",
				'dataType': 'json',
			},
			...
			columns: [{
					...
					render: function(data, row, columns, meta) {
						return '<a href="<?php echo (base_url('guru/show') . '/') ?>' + data + '">' + data + '</a>';
					}
				},
				{
					data: 'nama_atlet',					
		...
		$('#form-md-manage').submit(function(event) {
			$.post('<?php echo base_url('atlet/datadiri_save') ?>', $(this).serialize(), function(response, textStatus, xhr) {
		...
		$('#form-md-delete').submit(function(event) {
			$.post('<?php echo base_url('atlet/datadiri_delete') ?>', $(this).serialize(), function(response, textStatus, xhr) {
				tabel.ajax.reload();
				if (response.status == true) {
					toastr.success(response.msg);
	...
	function dt_add(t) {
		$.get('<?php echo base_url('atlet/modal/add') ?>').done(function(data) {
	...
	function dt_edit(t) {
    $.get('<?php echo base_url('/atlet/modal/edit') ?>', {
      'id': $(t).attr('target-id')
    }).done(function(data) {
	...
	function dt_delete(t) {
		$.get('<?php echo base_url('atlet/modal/delete') ?>', {
			'id': $(t).attr('target-id')
		}).done(function(data) {
