...
<script type="text/javascript">
	$(document).ready(function() {
		var tabel = $('#tabel-data').DataTable({
			ajax: {
				url: "<?php echo base_url('gejala/fetch') ?>",
				dataSrc: "data",
				"type": "POST",
				'dataType': 'json',
			},
		...
		$('#form-md-delete').submit(function(event) {
			$.post('<?php echo base_url('gejala/delete') ?>', $(this).serialize(), function(response, textStatus, xhr) {
				tabel.ajax.reload();
				if (response.status == true) {
					toastr.success(response.msg);
	...
	function dt_add(t) {
		$.get('<?php echo base_url('gejala/modal/add') ?>').done(function(data) {
	...
	function dt_edit(t) {
		$.get('<?php echo base_url('/gejala/modal/edit') ?>', {
			'kode_gejala': $(t).attr('target-id')
		}).done(function(data) {
	...
	function dt_delete(t) {
		$.get('<?php echo base_url('gejala/modal/delete') ?>', {
			'kode_gejala': $(t).attr('target-id')
		}).done(function(data) {
