...
<script type="text/javascript">
	$(document).ready(function() {
		var tabel = $('#tabel-data').DataTable({
			ajax: {
				url: "<?php echo base_url('pengetahuan/fetch') ?>",
				dataSrc: "data",
				"type": "POST",
				'dataType': 'json',
			},
				...
				},
				{
					...
					render: function(data, row, columns, meta) {
						...
						dt_btn_utils.find('.dt-edit').attr({
							'target-id': data.kode_pengetahuan,
		...
		$('#form-md-manage').submit(function(event) {
			$.post('<?php echo base_url('pengetahuan/save') ?>', $(this).serialize(), function(response, textStatus, xhr) {
		...
		$('#form-md-delete').submit(function(event) {
			$.post('<?php echo base_url('pengetahuan/delete') ?>', $(this).serialize(), function(response, textStatus, xhr) {
				tabel.ajax.reload();
				if (response.status == true) {
					toastr.success(response.msg);
	...
	function dt_add(t) {
		$.get('<?php echo base_url('pengetahuan/modal/add') ?>').done(function(data) {
	...
	function dt_edit(t) {
    $.get('<?php echo base_url('/pengetahuan/modal/edit') ?>', {
      'kode_pengetahuan': $(t).attr('target-id')
    }).done(function(data) {
	...
	function dt_delete(t) {
		$.get('<?php echo base_url('pengetahuan/modal/delete') ?>', {
			'kode_pengetahuan': $(t).attr('target-id')
		}).done(function(data) {
